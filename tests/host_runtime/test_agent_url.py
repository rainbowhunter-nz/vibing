...
def test_resolve_replaces_host_docker_internal(monkeypatch) -> None:
    ...
    assert resolve_agent_control_plane_url(url) == "ws://10.89.0.2:8080/api/v1/runtime/agent/ws"
...
def test_resolve_replaces_host_containers_internal(monkeypatch) -> None:
    ...
    assert resolve_agent_control_plane_url(url) == "ws://10.88.0.1:8080/api/v1/runtime/agent/ws"
