...
from vibing_api.core.discovery import DiscoveredDevcontainer, devcontainer_id
...
def test_list_merges_and_tags_source() -> None:
    record = _manual("m1", "/a")
    disc = DiscoveredDevcontainer(devcontainer_id("/b"), "b", "/b")
...
def test_manual_hides_discovered_with_same_path() -> None:
    ...
    disc = DiscoveredDevcontainer(devcontainer_id("/shared"), "shared", "/shared")
    cat = _catalog([record], [disc])
    items = cat.list()
