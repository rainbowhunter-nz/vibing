...
- `mcp_server.py`: `build_mcp_server` — streamable-HTTP MCP server (`list_harnesses`/`spawn`/`list_runs`/`get_run`/`await_run`/`stop`) the main harness calls (ADR-0011, ADR-0013). `list_harnesses` reads `vibing_harness` descriptors via `LocalExecutor`. `await_run` long-polls until a detached run is terminal (`completed`/`failed`/`stopped`) so the main harness can be woken by a background-shell waiter instead of polling. The server carries `instructions` framing these as *external subagents* and the rule-of-thumb model routing (see `docs/external-subagent-guide.md`).
