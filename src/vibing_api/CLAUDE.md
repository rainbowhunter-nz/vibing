...
- `core/live_state.py`: `LiveStateStore` — in-memory transient lifecycle status (starting/stopping/error) + last-pushed harness-status cache. Evicted on runtime disconnect (→ unknown, frontend `?`). Never persisted; lost on restart by design.
