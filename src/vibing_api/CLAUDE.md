...
- `core/runtime_injector.py`: two-phase `inject()` — `_bootstrap` (`docker cp` uv + wheel, synchronous `uv tool install` teed to `/tmp/vibing-runtime.log`, then a `vibing runtime preflight` HTTP probe of the control-plane `/api/v1/health`) then, **only on success**, `_spawn` (reap any prior runtime — it would still hold the MCP port and break the new bind — then a detached `nohup` launch with PID in `/tmp/vibing-runtime.pid`). Two separate `devcontainer exec` calls; the same control-plane URL (`host.docker.internal`, reached via host-gateway) is used by both the preflight and the launch. `inject*` return `bool` (bootstrap+preflight ok). Also `stop_runtime` (`<engine> exec` kill via PID file) and `stream_log` (async generator, `tail -n +1 -f` via injectable `log_streamer`).
