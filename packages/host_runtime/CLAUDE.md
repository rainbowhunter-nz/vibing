...
- `src/vibing_host_runtime/cli.py` — the `vibing-host-runtime` entry point: a Typer `cli` (single flat command, rich-rendered help); `run_worker` builds the `host_runtime_worker` register envelope, wires the adapter as the command handler, and runs a `RuntimeChannelClient` (from `vibing_runtime_client`); `main` is the console script.
- `src/vibing_host_runtime/client.py` — host-specific config only: `WorkerConfig` plus the `DEFAULT_CONTROL_PLANE_URL`/`DEFAULT_DEVCONTAINER_CLI` constants. The transport itself (`RuntimeChannelClient`, `Backoff`) lives in `vibing_runtime_client` and is re-exported from this package's `__init__`.
