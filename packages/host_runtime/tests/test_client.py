"""Tests for the Host Runtime Worker client — no real Control Plane or network."""
...
def _config() -> WorkerConfig:
    return WorkerConfig(control_plane_url="ws://test/ws", devcontainer_cli="devcontainer")


def _stop_after(
    client: HostRuntimeClient, n: int
...
def test_registers_then_handles_received_command() -> None:
    ...
    client = HostRuntimeClient(_config(), handler=handler, connect=FakeConnect([ws]))
...
def test_consumer_processes_commands_serially_in_fifo_order() -> None:
    ...
    async def scenario() -> None:
        client = HostRuntimeClient(_config(), handler=handler)
...
def test_emit_sends_runtime_event_envelope() -> None:
    async def scenario() -> None:
        client = HostRuntimeClient(_config())
        ws = FakeWS([])
...
def test_reconnects_with_bounded_backoff_after_failures() -> None:
    ...
    client = HostRuntimeClient(_config(), connect=connect)
...
def test_in_flight_command_not_replayed_after_reconnect() -> None:
    ...
    client = HostRuntimeClient(_config(), handler=handler, connect=connect)
