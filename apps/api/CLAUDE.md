...
- `src/vibing_api/api/routes/` — HTTP endpoints; thin, no SQL, raise HTTP 404s here.
  ...
  - `agent_sessions.py` — `POST /devcontainers/{id}/agent-sessions` (guards: DC exists → running → agent connected → no active session; creates row as `starting`, sends `start_agent_session` command to agent, returns 202).
...
- `src/vibing_api/core/` — domain core.
  ...
  - `config.py` — `VIBING_`-prefixed settings. `database.py` — connection helper. `errors.py` — error envelope + handlers (`AGENT_SESSION_ACTIVE` code + `ActiveAgentSessionError`; `RuntimeUnavailableError` accepts optional message). `commands.py` — re-export shim for `vibing_protocol.commands`.
