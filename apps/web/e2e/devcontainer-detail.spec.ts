...
//  dc-seed-0003 stopped  discovered has a seeded harness entry, but stopped → rows hidden ("container not running")
...
// Harness status is container-scoped: a stopped container hides cached rows entirely
// (dc-seed-0003 has a seeded harness entry but is stopped).
test('stopped container hides cached harness rows', async ({ page }) => {
  await page.goto('/devcontainers/dc-seed-0003')
  await expect(page.getByText(/container not running/i).first()).toBeVisible()
  await expect(page.getByTitle('Install claude-code', { exact: true })).toHaveCount(0)
})
...
// Runs against dc-seed-0001 (running) since install is only offered on a running container.
test('install action drives harness to Installed via SSE refetch', async ({ page }) => {
  await page.goto('/devcontainers/dc-seed-0001') // running; cursor not installed
